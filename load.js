...
way.prototype.Lay = function(preterms) {
    ...
    var maxLevel = Math.ceil(Math.log2(this.width/this.tileSize))
    this.maxLevel = Math.min(this.maxLevel, maxLevel);
...
way.prototype.Lay.prototype.out = function(preterms) {

    var base = new Object();
    this.over = new Image();
    this.over.src = this.getTile(0,0,0)+"&segmentation=y&segcolor=y";
    Object.assign(base,this).getTileUrl = this.getTile.bind(base);
    Object.assign(preterms, {id: this.id, tileSources: base});

    this.over.onload = this.bindSea(preterms);
};

way.prototype.Lay.prototype.bindSea = function (terms) {

  var seer = OpenSeadragon(terms);
  var overlay = seer.canvasOverlay({
    onRedraw: () => {
      overlay.context2d().globalAlpha = 0.4;
      overlay.context2d().drawImage(this.over,0,0,this.width,this.height);
    },
